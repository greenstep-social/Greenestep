...
function displayProfilePage() {
    ...
    if (user && recommendations) {
        ...
        recommendations.forEach((advice, index) => {
            if (advice.trim()) {
                const li = document.createElement("li");
                li.textContent = `Рекомендация ${index + 1}: ${advice.replace(/<br>/g, " ")}`;
                recommendationsList.appendChild(li);
            }
        });
    } else {
        alert("Ошибка: данные пользователя или рекомендации не найдены.");
    }
}
